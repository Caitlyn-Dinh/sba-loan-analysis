...
# Create a dummy regression dataset for demonstration
...
X_reg = pd.DataFrame(X_reg, columns=[f'feature_{i}' for i in range(X_reg.shape[1])]) # Convert to DataFrame for consistency
...
class SupervisedLearningFlow(FlowSpec):
    ...
    def train_model(self):
        ...
        algorithm = self.algorithms[self.current_algorithm] # self.algorithms is available here because it was set in 'start'
                                                            # and 'preprocess_data' is a linear step
    ...
    def aggregate_results(self, inputs):
        ...
        # FIX: Explicitly propagate self.algorithms to ensure it's available in subsequent steps
        # after the foreach/join. We can get it from any of the input tasks since it's the same.
        self.algorithms = inputs[0].algorithms 
    ...
    def generate_insights(self):
        ...
        if hasattr(self.tuned_model, 'feature_importances_'):
            # Ensure X.columns is available for feature naming
            if isinstance(self.X, pd.DataFrame):
                feature_names = self.X.columns
            else: # Fallback for numpy array if needed
                feature_names = [f"feature_{i}" for i in range(self.X.shape[1])]

            feature_importance = dict(zip(feature_names, self.tuned_model.feature_importances_))
